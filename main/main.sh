...
PH_TOTAL_RESULT=""
PH_RESULT=""
PH_ALL_ROLLBACK_PARAMS="PH_ROLLBACK_PARAMS PH_DEPTH_PARAMS PH_DEPTH PH_CONFIGURED_STATE PH_UNCONFIGURED_STATE PH_GROUPS PH_REMOVE_ACLS_USERS PH_CREATE_ACLS_USERS \
...
do
	...
	then
		...
		then
			printf "\n%2s\033[31m%s\033[0m\n\n" "" "ABORT : Reinstallation of PieHelper is required (Could not load critical codebase file '${PH_i}')"
			exit 1
		fi
	else
		printf "\n%2s\033[31m%s\033[0m\n\n" "" "ABORT : Reinstallation of PieHelper is required (Missing or unreadable critical codebase file '${PH_i}')"
		exit 1
	fi
done
...
for PH_i in $(nawk -v confdir="${PH_CONF_DIR}/" 'BEGIN { \
		ORS = " " \
	} { \
