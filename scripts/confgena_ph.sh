...
do
	case "${PH_OPTION}" in v)
		...
		>&2 printf "%27s%s\n" "" "- 'PieHelper-[timestamp]-snapshot-${PH_PIEH_VERSION}.tar' for configuration snapshots"
		>&2 printf "%27s%s\n" "" "- 'PieHelper-[timestamp]-build-[version].tar' for developer builds"
		...
		>&2 printf "%24s%s\n" "" "- If PieHelper option 'PH_PIEH_CIFS_SHARE' is set to 'yes', a backup of the generated archive will be created :"
		...
		>&2 printf "%21s%s\n" "" "\"rem\" allows removing the last modified tar archive of a specified type and a version [version]"
		>&2 printf "%24s%s\n" "" "- If PieHelper option 'PH_PIEH_CIFS_SHARE' is set to 'yes' and a CIFS backup of the matched archive"
		...
		>&2 printf "%24s%s\n" "" "- Archives will be created or removed from '${PH_SNAPSHOT_DIR}'"
		>&2 printf "%21s%s\n" "" "\"build\" which refers to developer builds"
		>&2 printf "%24s%s\n" "" "- Archives will be created or removed from '${PH_BUILD_DIR}'"
		>&2 printf "%24s%s\n" "" "- If a non-empty commit message [msg] is specified, developer builds will be :"
		>&2 printf "%27s%s\n" "" "- Committed to the local git repository using commit message [msg]"
		>&2 printf "%18s%s\n" "" "- Tagged as [version]"
		>&2 printf "%15s%s\n" "" "-v allows specifying a version number [version]"
		>&2 printf "%18s%s\n" "" "- Supported values for [version] are :"
		>&2 printf "%21s%s\n" "" "- [int] or [int].[int] where [int] is a positive integer"
		>&2 printf "%15s%s\n" "" "-d allows generation of developer builds for which the update to [version] from previous versions cannot be done from the PieHelper menu"
		>&2 printf "%18s%s\n" "" "- Creating builds where auto-update is denied is optional"
		>&2 printf "%18s%s\n" "" "- Auto-update is allowed by default"
		>&2 printf "%15s%s\n" "" "-g allows pushing developer builds to the master repository as well if a non-empty commit message [msg] is used"
		>&2 printf "%18s%s\n" "" "- Master commits :"
