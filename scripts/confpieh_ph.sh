...
printf "\033[36m%s\033[0m\n\n" "- Checking the validity of requested modules"
...
case "${PH_ACTION}" in repair)
	...
	do
		...
		case "${PH_i}" in *.sh)
			...
			then
				if echo -n "${PH_PIEH_DEBUG}" | grep ',' >/dev/null
				then
	...
	case "${PH_MODULES}" in prompt)
		...
		then
			PH_REQUESTED="${PH_MODULES}"
			PH_MODULES="$(cut -c1<<<"${PH_MODULES}" | tr '[:lower:]' '[:upper:]')$(cut -c2-<<<"${PH_MODULES}")"
			for PH_i in $("${PH_SUDO}" find "${PH_BASE_DIR}" -mount \( -name "*.sh" -or -name "*.expect" \) 2>/dev/null | sort)
			do
				PH_i="${PH_i##*/}"
				if [[ "${PH_i}" == *.expect ]]
				then
