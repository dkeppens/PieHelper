#!/bin/ksh
# Start Moonlight (by Davy Keppens on 04/10/2018)
# Enable/Disable debug by running confpieh_ph.sh -d startmoon.sh
...
do
        case $PH_OPTION in *)
                >&2 printf "%s\n" "Usage : startmoon.sh | -h"
                >&2 printf "\n"
                >&2 printf "%3s%s\n" "" "Where -h displays this usage"
                >&2 printf "%9s%s\n" "" "- Running this script without parameters will start an instance of Moonlight on it's allocated TTY if one is not already running"
