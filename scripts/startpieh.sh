...
do
        case "$PH_OPTION" in p)
                [[ `tty` != /dev/pts/* ]] && printf "\033[36m%s\033[0m\n" "- Enabling '$PH_RUNAPP'" && printf "%2s\033[31m%s\033[0m\n\n" "" "FAILED : Not currently on a pseudo-terminal" && \
...
then
	...
	do
		...
		then
			...
			PH_RUNAPP_CMD=`nawk -v runapp=^"$PH_i"$ 'BEGIN { ORS=" " } $1 ~ runapp { for (i=2;i<=NF;i++) { if (i==NF) { ORS="" ; print $i } else { print $i }}}' "$PH_CONF_DIR"/supported_apps 2>/dev/null`
...
then
	...
	then
		...
		then
			...
			ph_set_option PieHelper -r PH_PIEH_PERSISTENT='yes' || (printf "%2s\033[31m%s\033[0m\n\n" "" "FAILED" ; return 1) || exit "$?"
			ph_run_app_action start "$PH_RUNAPP"
			[[ "$?" -eq 0 ]] && printf "%2s\033[32m%s\033[0m\n\n" "" "SUCCESS" || (printf "%2s\033[31m%s\033[0m\n\n" "" "FAILED" ; return 1) || exit "$?"
		fi
	else
		if pgrep -t tty"$PH_RUNAPP_TTY" -f start"$PH_RUNAPPL".sh >/dev/null
		then
        		printf "%10s\033[32m%s\033[0m\n" "" "OK ('TTY$PH_RUNAPP_TTY') -> Stopping"
			printf "%2s\033[32m%s\033[0m\n\n" "" "SUCCESS"
