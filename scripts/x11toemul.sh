#!/bin/ksh
# Move from X11 to Emulationstation (by Davy Keppens on 04/10/2018)
# Enable/Disable debug by running confpieh_ph.sh -d x11toemul.sh
...
do
		...
		>&2 printf "%9s%s\n" "" "  If successful, a new instance of $PH_RUNAPP will be started on it's allocated TTY"
                ...
                >&2 printf "%12s%s\n" "" "  To avoid unnecessary actions for move scripts, one exception is $PH_STOPAPP"
