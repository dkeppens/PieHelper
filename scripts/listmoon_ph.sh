...
do
	...
	printf "%s\n" "$((PH_TOTAL+1)). Exit"
	printf "\n"
	printf "%2s%s" "" "Your choice : "
	read PH_ANSWER 2>/dev/null
	((PH_COUNT++))
done
if [[ "$PH_ANSWER" -ne "$((PH_TOTAL+1))" ]]
then
	PH_GAME=`"$PH_SUDO" -E su "$PH_MOON_USER" -c "LD_LIBRARY_PATH=/usr/local/lib:/usr/lib:/lib $PH_MOON_PATH list $PH_MOON_SRV | tail -n +2" | nawk -v game=$PH_ANSWER 'NR ~ game { print $2 ; exit 0 } { next }' 2>/dev/null`
	printf "%2s\033[32m%s\033[0m\n\n" "" "SUCCESS : '$PH_GAME'"
	confopts_ph.sh -p set -a Moonlight -o PH_MOON_GAME="$PH_GAME" || (printf "2%s\033[31m%s\033[0m\n\n" "" "FAILED" ; return 1) || exit "$?"
else
	printf "%2s\033[32m%s\033[0m\n\n" "" "SUCCESS"
fi
exit 0
