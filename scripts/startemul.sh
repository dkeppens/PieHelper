#!/bin/ksh
# Start Emulationstation (by Davy Keppens on 04/10/2018)
# Enable/Disable debug by running confpieh_ph.sh -d startemul.sh

. $(dirname $0)/../main/main.sh || exit $? && set +x

#set -x

typeset PH_RUNAPP="Emulationstation"
...
do
        case $PH_OPTION in *)
                >&2 printf "%s\n" "Usage : startemul.sh | -h"
                >&2 printf "\n"
                >&2 printf "%3s%s\n" "" "Where -h displays this usage"
                >&2 printf "%9s%s\n" "" "- Running this script without parameters will start an instance of Emulationstation on it's allocated TTY if one is not already running"
