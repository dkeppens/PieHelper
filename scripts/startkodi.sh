#!/bin/ksh
# Start Kodi (by Davy Keppens on 04/10/2018)
# Enable/Disable debug by running confpieh_ph.sh -d startkodi.sh
...
do
        case $PH_OPTION in *)
                >&2 printf "%s\n" "Usage : startkodi.sh | -h"
                >&2 printf "\n"
                >&2 printf "%3s%s\n" "" "Where -h displays this usage"
                >&2 printf "%9s%s\n" "" "- Running this script without parameters will start an instance of Kodi on it's allocated TTY if one is not already running"
